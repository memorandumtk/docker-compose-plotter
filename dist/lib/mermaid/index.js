...
class ComposeMermaidGenerator {
            ...
            if (Array.isArray(ports)) {
                ports.forEach((port) => {
                    if (typeof port === "string") {
                        ...
                        portsArray.push(this.escapeEnvVariables(port.target ?? "") +
                            ":" +
    ...
    generateMermaidDiagram() {
        ...
        this.serviceNodes.forEach((node, serviceName) => {
            const isInSubgraph = Array.from(this.networkSubgraphsMap.values()).some((subgraphData) => subgraphData.services.has(serviceName));
            if (!isInSubgraph) {
